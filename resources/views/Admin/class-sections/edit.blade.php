...
<div class="container mx-auto px-4 py-6">
    <h1 class="text-2xl font-bold mb-4">Edit Class Section</h1>
    <form method="POST" action="{{ route('admin.class-sections.update', $classSection) }}">
        ...
        <div class="mb-4">
            ...
            <select name="class_id" class="mt-1 block w-full border rounded-md px-3 py-2" required>
                @foreach($classes as $class)
                    <option value="{{ $class->id }}" {{ old('class_id', $classSection->class_id) == $class->id ? 'selected' : '' }}>{{ $class->full_name }}</option>
                @endforeach
            </select>
        ...
        <div class="mb-4">
            ...
            <input type="text" name="section_name" class="mt-1 block w-full border rounded-md px-3 py-2" value="{{ old('section_name', $classSection->section_name) }}" required>
            @error('section_name') <p class="text-red-500 text-xs">{{ $message }}</p> @enderror
        </div>
        <div class="mb-4">
            <label class="block text-sm font-medium">Capacity</label>
            <input type="number" name="capacity" class="mt-1 block w-full border rounded-md px-3 py-2" value="{{ old('capacity', $classSection->capacity) }}">
            @error('capacity') <p class="text-red-500 text-xs">{{ $message }}</p> @enderror
        </div>
        <div class="mb-4">
            <label class="block text-sm font-medium">Student Strength (manual)</label>
            <input type="number" name="student_strength" class="mt-1 block w-full border rounded-md px-3 py-2" value="{{ old('student_strength', $classSection->student_strength) }}">
