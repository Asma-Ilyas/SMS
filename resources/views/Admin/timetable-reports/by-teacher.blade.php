...
<div class="min-h-screen bg-gray-50/50 py-8">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="flex flex-col md:flex-row md:justify-between md:items-center gap-4 mb-6 pb-5 border-b border-gray-200">
            <div>
                <h1 class="text-2xl font-bold text-gray-900">👨‍🏫 Teacher-wise Timetable</h1>
            ...
            <div class="flex flex-wrap gap-2">
                ...
                @if(isset($teacherId) && $teacherId > 0 && isset($activeTiming) && $activeTiming)
                    <a href="{{ route('admin.timetable-reports.print', ['teacher_id' => $teacherId]) }}"
                       target="_blank" class="px-4 py-2 text-sm bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition">🖨 Print</a>
                    <a href="{{ route('admin.timetable-reports.export', ['teacher_id' => $teacherId]) }}"
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200/80 p-5 mb-6">
            <form method="GET" action="{{ route('admin.timetable-reports.teacher') }}" class="flex flex-wrap items-end gap-4">
                <div class="flex-1 min-w-[200px]">
                    ...
                    <select name="teacher_id" class="w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                        ...
                        @foreach($teachers ?? [] as $teacher)
                            <option value="{{ $teacher->id }}" {{ ($teacherId ?? 0) == $teacher->id ? 'selected' : '' }}>
                ...
                <div class="flex-1 min-w-[150px]">
                    ...
                    <select name="day" class="w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                        @foreach($days ?? ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday'] as $d)
                            <option value="{{ $d }}" {{ ($day ?? 'Monday') == $d ? 'selected' : '' }}>{{ $d }}</option>
                        @endforeach
                    </select>
                </div>
        ...
        {{-- Timetable Display --}}
        @if(isset($selectedTeacher) && $selectedTeacher && isset($activeTiming) && $activeTiming)
            <div class="bg-white rounded-xl shadow-sm border border-gray-200/80 overflow-hidden">
                <div class="px-6 py-4 border-b border-gray-200 bg-gray-50 flex justify-between items-center">
                    <h2 class="text-lg font-bold text-gray-800">
                        {{ $selectedTeacher->full_name ?? $selectedTeacher->name ?? 'Teacher' }}
                        <span class="text-sm font-normal text-gray-500 ml-2">
                            ({{ $entries->count() ?? 0 }} classes)
                        </span>
                    </h2>
                                            ...
                                            @if($slot->type === 'period')
                                                @if($entry)
                                                    <div class="bg-white border border-gray-200 rounded-lg p-2 hover:shadow-md transition shadow-sm">
                                                        ...
                                                        <div class="text-xs text-gray-600 mt-0.5">{{ $entry->classSection->full_name ?? $entry->classSection->section_name ?? 'N/A' }}</div>
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mt-6">
                <div class="bg-white rounded-xl shadow-sm border border-gray-200/80 p-4">
                    ...
                    <p class="text-2xl font-bold text-gray-900">{{ $entries->groupBy('class_section_id')->count() ?? 0 }}</p>
                </div>
                <div class="bg-white rounded-xl shadow-sm border border-gray-200/80 p-4">
                    <p class="text-xs text-gray-500">Total Periods</p>
                    <p class="text-2xl font-bold text-gray-900">{{ $entries->where('timeSlot.is_break', false)->count() ?? 0 }}</p>
                </div>
                <div class="bg-white rounded-xl shadow-sm border border-gray-200/80 p-4">
                    <p class="text-xs text-gray-500">Breaks</p>
                    <p class="text-2xl font-bold text-gray-900">{{ $entries->where('timeSlot.is_break', true)->count() ?? 0 }}</p>
                </div>
                <div class="bg-white rounded-xl shadow-sm border border-gray-200/80 p-4">
                    <p class="text-xs text-gray-500">Days Covered</p>
                    <p class="text-2xl font-bold text-gray-900">{{ $entries->groupBy('day_of_week')->count() ?? 0 }}/5</p>
                </div>
            </div>

        @elseif(isset($teacherId) && $teacherId > 0 && !isset($activeTiming))
            <div class="bg-red-50 border-l-4 border-red-400 p-4 rounded-lg">
                <p class="text-sm text-red-700">⚠️ No active timing session found. Please activate a timing session first.</p>
            </div>
        @elseif(isset($teacherId) && $teacherId > 0)
            <div class="bg-yellow-50 border-l-4 border-yellow-400 p-4 rounded-lg">
                <p class="text-sm text-yellow-700">💡 No timetable entries found for this teacher.</p>
                <p class="text-sm text-yellow-600 mt-1">Please generate the timetable or assign classes to this teacher.</p>
            </div>
        @else
            <div class="bg-blue-50 border-l-4 border-blue-400 p-4 rounded-lg">
                <p class="text-sm text-blue-700">💡 Please select a teacher and click "Filter" to view the timetable.</p>
            </div>
        @endif
