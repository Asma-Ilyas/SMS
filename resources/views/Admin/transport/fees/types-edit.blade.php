...
<div class="p-6 max-w-2xl">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
        <form action="{{ route('admin.transport-fee-types.update', $feeType) }}" method="POST">
            @csrf
            @method('PUT')
                ...
                <div class="md:col-span-2">
                    ...
                    <select name="route_id" class="w-full rounded-xl border border-gray-300 px-3 py-2 text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 outline-none">
                        ...
                        @foreach($routes as $route)
                            <option value="{{ $route->id }}" @selected(old('route_id', $feeType->route_id)==$route->id)>{{ $route->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
