...
<div class="py-8 px-4 sm:px-6 lg:px-8">
    ...
    <div class="mb-8 flex flex-wrap justify-between items-center gap-4">
        <div>
            <h1 class="text-3xl font-bold text-gray-900 tracking-tight">Floors</h1>
            <p class="text-gray-500 text-sm mt-1">Organize floors across blocks and buildings</p>
        </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-6 flex items-center justify-between transition hover:shadow-md">
            <div>
                ...
                <p class="text-3xl font-bold text-gray-900 mt-1">{{ $floors->total() }}</p>
            </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-6 flex items-center justify-between transition hover:shadow-md">
            <div>
                <p class="text-sm font-medium text-gray-500">Blocks</p>
                <p class="text-3xl font-bold text-gray-900 mt-1">{{ \App\Models\Block::count() }}</p>
            </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-6 flex items-center justify-between transition hover:shadow-md">
            <div>
                <p class="text-sm font-medium text-gray-500">Floors with Rooms</p>
                <p class="text-3xl font-bold text-gray-900 mt-1">{{ $floors->filter(fn($f) => $f->rooms_count > 0)->count() }}</p>
            </div>
        ...
        <form method="GET" class="flex flex-wrap gap-3 items-end">
            <div class="flex-1 min-w-[200px]">
                ...
                <div class="relative">
                    ...
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Floor name or code..." class="pl-9 w-full rounded-lg border-gray-300 text-sm focus:ring-indigo-500 focus:border-indigo-500">
                </div>
            </div>
            <div class="w-48">
                <label class="block text-xs font-medium text-gray-600 mb-1">Block</label>
                <select name="block_id" class="w-full rounded-lg border-gray-300 text-sm focus:ring-indigo-500 focus:border-indigo-500">
            ...
            <div class="flex gap-2">
                ...
                <a href="{{ route('admin.floors.index') }}" class="inline-flex items-center px-4 py-2 bg-white border border-gray-300 text-gray-700 text-sm font-medium rounded-lg shadow-sm hover:bg-gray-50">
                    Clear
                </a>
            </div>
        </form>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-200 flex justify-between items-center bg-gray-50/40">
            ...
            <span class="text-xs text-gray-500 bg-gray-100 px-2 py-1 rounded-full">{{ $floors->total() }} total</span>
        </div>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50/50 transition-colors duration-150">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            @if($floor->code)
                                <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-gray-100 text-gray-800">{{ $floor->code }}</span>
                            @else
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                            @php
                                $levelIcon = $floor->level == 0 ? '🏠' : ($floor->level > 0 ? '⬆️' : '⬇️');
                            @endphp
                            {{ $levelIcon }} {{ $floor->level }}
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            @if($floor->block)
                                <span class="inline-flex items-center gap-1">
                                    <span class="w-2 h-2 rounded-full bg-indigo-400"></span>
                                    <span class="text-sm text-gray-700">{{ $floor->block->name }}</span>
                                </span>
                            @else
                                <span class="text-gray-400 text-sm">—</span>
                            @endif
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            @php $roomsCount = $floor->rooms_count ?? $floor->rooms->count(); @endphp
                            @if($roomsCount > 0)
                            ...
                            @else
                                <span class="text-gray-400 text-sm">0 rooms</span>
                            @endif
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <div class="flex items-center justify-end gap-2">
                                ...
                                <form action="{{ route('admin.floors.destroy', $floor) }}" method="POST" class="inline-block">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" 
                                            class="inline-flex items-center justify-center w-8 h-8 rounded-lg text-red-600 hover:bg-red-50 transition-colors"
                                            onclick="return confirm('Delete this floor? It will remove the floor reference from rooms.')"
                                            title="Delete">
                    ...
                    <tr>
                        <td colspan="6" class="px-6 py-12 text-center">
                            ...
                            <p class="mt-2 text-sm text-gray-500">No floors found.</p>
                            <a href="{{ route('admin.floors.create') }}" class="mt-3 inline-flex items-center px-3 py-1.5 text-sm font-medium text-indigo-600 hover:text-indigo-500">Create one →</a>
                        </td>
                    </tr>
                    @endforelse
                </tbody>
