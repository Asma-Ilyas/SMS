...
<div class="p-6">
    <div class="flex justify-between items-center mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Hostel Fee Types</h1>
            <p class="text-sm text-gray-500 mt-1">Define recurring hostel fee amounts.</p>
        </div>
        <a href="{{ route('admin.hostel-fee-types.create') }}" class="inline-flex items-center px-4 py-2.5 bg-indigo-600 text-white text-sm font-medium rounded-xl shadow-lg shadow-indigo-500/25 hover:bg-indigo-700 transition">+ Add Fee Type</a>
    </div>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4 text-sm text-gray-600">{{ $ft->hostel?->name ?? 'All Hostels' }}</td>
                        ...
                        <td class="px-6 py-4 text-right text-sm space-x-2">
                            <a href="{{ route('admin.hostel-fee-types.edit', $ft) }}" class="text-gray-600 hover:text-gray-900 font-medium">Edit</a>
                            <form action="{{ route('admin.hostel-fee-types.destroy', $ft) }}" method="POST" class="inline" onsubmit="return confirm('Delete this fee type?')">
    ...
    <div class="mt-6">
        <a href="{{ route('admin.hostel-fee-payments.index') }}" class="inline-flex items-center px-4 py-2 border border-indigo-200 text-indigo-700 text-sm font-medium rounded-xl hover:bg-indigo-50 transition">View Fee Payments →</a>
    </div>
</div>
@endsection
